class AtendeesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @atendee.save
        ...
        format.json { render action: 'show', status: :created, location: @atendee }
      else
  ...
  def destroy
    @atendee.destroy
    respond_to do |format|
      format.html { redirect_to atendees_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def atendee_params
      params.require(:atendee).permit(:name, :attendance, :comment)
    end
end
